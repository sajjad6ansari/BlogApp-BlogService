...
export const addComment = TryCatchHandler(async (req: AuthenticatedRequest, res) => {
...
export const getAllComments = TryCatchHandler(async (req, res) => {
  ...
  const comments =
    await sql`SELECT * FROM comments WHERE blog_id = ${id} ORDER BY created_at DESC`;
...
export const saveBlog = TryCatchHandler(async (req: AuthenticatedRequest, res) => {
...
export const getSavedBlog = TryCatchHandler(async (req: AuthenticatedRequest, res) => {
  const blogs =
    await sql`SELECT * FROM savedblogs WHERE user_id = ${req.user?._id}`;
